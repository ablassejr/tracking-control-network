...
parent: ""
collections: []
'This report provides a comprehensive survey of noise models, adversarial attack techniques, simulation frameworks, benchmark problems, and recent research (2023--2026) relevant to defending neural network-based tracking controllers against adversarial attacks. The research covers discrete-time affine-in-the-inputs systems of the form $x(k+1) = f(x(k)) + g(x(k))u(k)$, where neural network controllers compute optimal control inputs and adversarial attacks target sensor measurements, actuator signals, or the neural network itself. Key findings include': '(1) a well-established taxonomy of noise and attack models ranging from Gaussian white noise to sophisticated false data injection and replay attacks; (2) mature simulation toolkits in MATLAB (Robust Control Toolbox, Deep Learning Toolbox, NNV) and Python (ART, Foolbox, python-control); (3) emerging research at the intersection of reinforcement learning, adversarial training, and control-theoretic stability guarantees; and (4) practical IDS/ACC benchmarks directly relevant to this project''s analogy framework.'
$version: 0
$libraryID: 1
$itemKey: 3ULDQWPF
